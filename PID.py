...
class PID(object):
    ...
    def __call__(self, input_, dt=None, drop = False, pre = False, is_ov = False):
        """
        ...
        error = (err + self._last_error) / 2 if self._last_error is not None else err
        
        kk = 1.0
        inp1 = input_ if self._last_input is None else input_ * kk + self._last_input * (1-kk)
        d_input = inp1 - (self._last_input if (self._last_input is not None) else inp1)
        ...
        #self._proportional = _clamp(self._proportional, (-8,8))
        # Compute integral and derivative terms
        overshoot = 1 if is_ov else 1
        if not drop and not pre and error < 20:  #清除积分？
            self._integral += self.Ki * error * dt * overshoot

        self._integral = _clamp(self._integral, self.output_limits)  # Avoid integral windup

        self._derivative = -self.Kd * d_input / dt 
        print("deri",round(self._derivative,4),"d_input",d_input,"dt",dt,"input",input_,"last",self._last_input)
